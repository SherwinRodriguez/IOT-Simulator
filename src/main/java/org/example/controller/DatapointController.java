...
public class DatapointController {
        ...
        return deviceRepository.findByIdAndUser(deviceId, user).map(device -> {
            if (device.getZohoModelId() == null) {
                return ResponseEntity.badRequest()
                        .body(Map.of("error", "Device is missing Zoho Model ID. Please sync devices again."));
            }
            ...
            try {
                ...
                if (body != null && !body.trim().isEmpty()) {
                    ...
                    for (JsonNode dp : dpNodes) {
                        ...
                        String unit = (dp.has("unit") && dp.get("unit").has("name"))
                                ? dp.get("unit").get("name").asText()
                                : "";
                        ...
                        Optional<SimulationConfigEntity> existingCfg = localConfigs.stream()
                                .filter(c -> c.getParsingKey().equals(finalParsingKey)).findFirst();
        ...
        return deviceRepository.findByIdAndUser(deviceId, user).map(device -> {
            ...
            try {
                ...
                if (rawType.equals("string"))
                    ...
                    mappedDataType = "Boolean";
                else
                    mappedDataType = "Numeric";
                ...
                if (dpNodes != null && dpNodes.isArray()) {
                    ...
                    if (kindId == null && dpNodes.size() > 0 && dpNodes.get(0).has("kind")
                            && dpNodes.get(0).get("kind").has("id")) {
                ...
                String pKey = (request.parsingKey() != null && !request.parsingKey().isBlank()) ? request.parsingKey()
                ...
                return ResponseEntity
                        .ok(Map.of("message", "Datapoint created successfully in Zoho IoT", "parsingKey", pKey));
            } catch (Exception e) {
    ...
    public ResponseEntity<?> getConfig(@PathVariable UUID deviceId,
        ...
        return deviceRepository.findByIdAndUser(deviceId, user)
                .map(device -> simConfigRepository.findByDeviceIdAndParsingKey(deviceId, parsingKey)
    ...
    public record SimulationConfigRequest(
            @NotBlank(message = "Pattern cannot be blank") String pattern,
            @NotNull Double minValue,
            @NotNull Double maxValue,
            @NotNull Double startValue,
            @NotNull @Min(value = 0, message = "Step value must be non-negative") Double stepValue,
            @NotNull @Min(value = 500, message = "Publish interval must be at least 500ms") Integer publishIntervalMs) {
    }
